...
def k_accuracy(output, target, topk=(1,)):
    ...
    target_reshaped = tf.reshape(
        tf.repeat(target_reshaped, y_pred.shape[0]), (y_pred.shape[0], 1)
    )
    # _,target_reshaped = tf.transpose(tf.repeat(tf.split(tf.where(target),2,axis=1),y_pred.shape[0]))

    correct = y_pred == tf.cast(
        target_reshaped, tf.int32
    ...
    for k in topk:
        ...
        flattened_indicator_which_topk_matched_truth = tf.cast(
            tf.reshape(ind_which_topk_matched_truth, -1), tf.float32
        )  # [k, B] -> [kB]
        # get if we got it right for any of our top k prediction for each example in batch
        tot_correct_topk = tf.math.reduce_sum(
            tf.cast(flattened_indicator_which_topk_matched_truth, tf.float32),
            axis=0,
            keepdims=True,
        )  # [kB] -> [1]
